...
export default function Settings() {
  ...
  const handleEditUser = async (e) => {
    ...
    try {
      const res =await editUser({ credentials, id });
      console.log(res)
    } catch (err) {
      // console.log(err);
    }
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      setErrors({ message: error?.data || "An error occurred" });
    }
  ...
  useEffect(() => {
    if (passwordIsSuccess) {
      ...
      setPasswordErrors({
        message: passwordError?.data || "An error occurred",
      });
    }
  ...
  return (
    <div className="flex md:py-8 py-4 px-4 w-full">
      <div className="bg-white shadow-md rounded-md py-4 md:py-8 px-5 md:px-[50px] 2xl:px-[100px] w-full max-w-7xl mx-auto">
        ...
        >
          {/* {passwordErrors && <p className="text-red-500">{passwordErrors?.message}</p>} */}
